	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<header class="entry-header">
			...
			<?php if ( has_post_thumbnail() && !is_search() ) { ?>
				<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to ', 'f4d' ) . '%s', the_title_attribute( 'echo=0' ) ) ); ?>">
		...
		<?php }
			...
			<div class="entry-content">
				<?php if ( has_excerpt() ) {
					the_excerpt(); ?>
					<p><a class="more-link" href="<?php the_permalink(); ?>"><?php echo wp_kses( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'f4d' ), array( 'span' => array(
						'class' => array() ) ) ) ?></a></p>
				<?php }
				else {
					the_content( wp_kses( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'f4d' ), array( 'span' => array(
						'class' => array() ) ) )
						);
				}
				 ?>
