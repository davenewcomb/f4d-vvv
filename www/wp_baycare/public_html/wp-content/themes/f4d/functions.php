 ...
 */
if ( ! function_exists( 'f4d_fonts_url' ) ) {
...
// FROM U-Design   Setup footer's "Back to Top" link
function f4d_footer_back_to_top_link() {
    ob_start(); ?>
		    <div class="back-to-top">
			<a href="#top"><?php esc_html_e('Back to Top', 'f4d'); ?></a>
		    </div>
<?php
    $footer_back_to_top_html = ob_get_clean();
    $html = apply_filters( 'f4d_get_footer_back_to_top_link', $footer_back_to_top_html );
    echo $html;
}
add_action('f4d_footer_inside', 'f4d_footer_back_to_top_link', 10);


/**
 * FROM U-Design   Hide email from Spam Bots using a shortcode. Example: [safe_email]john.doe@example.com[/safe_email]
 *
 ...
 */
function f4d_hide_email_shortcode( $atts , $content = null ) {
	if ( ! is_email( $content ) ) {
		return;
	}
	return '<a href="mailto:' . antispambot( $content ) . '">' . antispambot( $content ) . '</a>';
}
add_shortcode( 'safe_email', 'f4d_hide_email_shortcode' );
...
<?php       // calculate top area height offset and custom styles
            ...
            if ( $top_area_over_header_image ) : ?>
                ...
                <style>@media screen and (min-width: 320px){#top-wrapper{background-color:<?php echo $page_top_area_bg; ?>;background-image: none;position:relative;}}@media screen and (max-width: 1440px){#page-custom-header img{margin-top:0 !important;}}</style>
<?php       endif; ?>
 ...
 * @return void
 */
if ( ! function_exists( 'f4d_scripts_styles' ) ) {
 ...
 * @return void
 */
if ( ! function_exists( 'f4d_content_width' ) ) {
 ...
 */
 if ( ! function_exists( 'f4d_new_contactmethods' ) ) {
